...
<head>
    ...
    <title>Edit Case - {{ config('app.name', 'Prahari') }}</title>
...
<body>
    ...
    <div class="container">
        <div class="header-section">
            <div>
                <h1>Edit Case</h1>
                <p class="subtitle">Update case details without leaving the workflow.</p>
            </div>
            <a href="{{ route('admin.dashboard') }}" class="btn-secondary">Back to Dashboard</a>
        </div>

        <form method="POST" action="{{ route('admin.cases.update', $case) }}" enctype="multipart/form-data" class="glass-card">
            @csrf
            @method('PUT')
            @include('admin.cases.form', ['case' => $case])
            <div class="form-actions">
                <a href="{{ route('admin.dashboard') }}" class="btn-secondary">Cancel</a>
                <button type="submit" class="btn-primary">Update Case</button>
            </div>
        </form>
    </div>
</body>
